...
Random_DiscreteDistribution * Random_DiscreteDistribution_Open() {
	Random_DiscreteDistribution* rPtr = new Random_DiscreteDistribution();
	return rPtr;
}

extern "C" __declspec(dllexport)
void Random_DiscreteDistribution_Close(Random_DiscreteDistribution * rPtr)
{
	delete rPtr;
}

extern "C" __declspec(dllexport)
int* Random_DiscreteDistribution_Run(Random_DiscreteDistribution * rPtr, int rows, int cols)
{
	rPtr->discrete = Mat(rows, cols, CV_32F, 0);
	return (int*)rPtr->discrete.data;
}