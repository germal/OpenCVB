...
extern "C" __declspec(dllexport)
void Salience_Close(Salience *s)
{
	delete s;
}

extern "C" __declspec(dllexport)
int *Salience_Run(Salience *s, int numScales, int *grayInput, int rows, int cols)
{
	s->numScales = numScales;
	if (s->matsReady == true) if (s->src.rows != rows && s->src.cols != cols) s->allocateMats(rows, cols);
	s->src = Mat(rows, cols, CV_8U, grayInput);
	if (s->matsReady == false) s->allocateMats(rows, cols);
	Mat gray32f = s->calcIntensity();

	normalize(gray32f, gray32f, 0, 255, NORM_MINMAX, CV_32F);
	gray32f.convertTo(s->dst, CV_8U);
	return (int *)s->dst.data;
}