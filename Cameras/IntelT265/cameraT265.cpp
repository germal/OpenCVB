...
{
public:
	Mat leftViewMap1, leftViewMap2, rightViewMap1, rightViewMap2, remapLeft, remapRight, color, RGBDepth, disparity, depth16, rightViewRaw, leftViewRaw;
	float *vertices;
	int width, height;
...
{
	tp->leftViewRaw = Mat(tp->rawHeight, tp->rawWidth, CV_8U, (void*)tp->frameset.get_fisheye_frame(1).get_data());
...
{
	tp->rightViewRaw = Mat(tp->rawHeight, tp->rawWidth, CV_8U, (void*)tp->frameset.get_fisheye_frame(2).get_data());
...
{
	auto fs = tp->frameset.as<rs2::frameset>();
	tp->leftViewRaw = Mat(tp->rawHeight, tp->rawWidth, CV_8U, (void*)fs.get_fisheye_frame(1).get_data());
	remap(tp->leftViewRaw, tp->color, tp->leftViewMap1, tp->leftViewMap2, INTER_LINEAR);
	resize(tp->color, tp->color, Size(tp->width, tp->height));
	cvtColor(tp->color, tp->color, COLOR_GRAY2BGR);
	tp->RGBDepth = tp->color.clone();

	//Mat disp16s, remapLeft, remapRight;
	//cv::remap(tp->leftViewRaw, remapLeft, tp->lm1, tp->lm2, INTER_LINEAR);
	//cv::remap(tp->rightViewRaw, remapRight, tp->rm1, tp->rm2, INTER_LINEAR);
	//disp16s = tp->sgm->Run(remapLeft, remapRight, tp->maxDisp);

	//Rect validRect = Rect(tp->maxDisp, 0, disp16s.cols - tp->maxDisp, disp16s.rows);
	//disp16s = disp16s(validRect);
	//disp16s.convertTo(tp->disparity, CV_32F, 1.0f / 16.0f);

	//Mat disp_vis = tp->disparity.clone();

	//Mat mask;
	//threshold(disp_vis, mask, 1, 255, THRESH_BINARY);
	//mask.convertTo(mask, CV_8U);
	//disp_vis *= 255.0 / tp->numDisp;

	//// convert disparity To 0 - 255 And color it
	//Mat tmpRGBDepth;
	//cv::convertScaleAbs(disp_vis, disp_vis, 1);
	//cv::applyColorMap(disp_vis, tmpRGBDepth, cv::COLORMAP_JET);
	//cv::Rect depthRect = Rect(int(tp->stereo_cx), 0, tmpRGBDepth.cols, tmpRGBDepth.rows);
	//tmpRGBDepth.copyTo(tp->RGBDepth(depthRect), mask);
	//tp->depth16 = tp->disparity.clone();
...
{
	tp->pose_data = tp->frameset.as<rs2::pose_frame>().get_pose_data();
...
{
	auto f = tp->frameset.first_or_default(RS2_STREAM_POSE);
...
{
	tp->frameset = tp->pipeline.wait_for_frames(1000);
	if (tp->frameset) return 1; else return 0;
}
#else
#include <iostream>
#include <iomanip>
