...
Public Class Rodrigues_ValidateKinect
    ...
    Public Sub Run(ocvb As AlgorithmData)
        If ocvb.parms.cameraIndex <> Kinect4AzureCam Then
            dst2.SetTo(0)
            ocvb.putText(New TTtext("Only the Kinect4Azure camera is currently supported for the Rodrigues calibration", 10, 140, RESULT1))
            Exit Sub
        End If
        ...
        For i = 0 To split.Length - 1
            ocvb.putText(New TTtext(split(i), 10, 90 + i * 20, RESULT1))
        Next
    End Sub
End Class
...
Public Class Rodrigues_ValidateVector
    ...
    Public Sub Run(ocvb As AlgorithmData)
        If ocvb.parms.cameraIndex <> StereoLabsZED2 And ocvb.parms.cameraIndex <> T265Camera Then
            dst2.SetTo(0)
            ocvb.putText(New TTtext("Only the StereoLabs Zed 2 and Intel T265 cameras are supported for this Rodrigues validation", 10, 140, RESULT1))
            Exit Sub
        End If
        ...
        Next
        ocvb.putText(New TTtext(output, 10, 90, RESULT1))
        ...
        Next
        ocvb.putText(New TTtext(output, 10, 150, RESULT1))
        ...
        ocvb.putText(New TTtext(output, 10, 190, RESULT1))

        If ocvb.parms.cameraIndex = T265Camera Then
            ocvb.putText(New TTtext("The T265 does not provide the Rotation Matrix but it is calculated from the Rotation Vector.", 10, 220, RESULT1))
        End If
    End Sub
End Class
