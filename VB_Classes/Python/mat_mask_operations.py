...
import numpy as np
import cv2 as cv
title_window = 'Mat_mask_operations.py'
import ctypes
...
def main(argv):
    ...
    dst = np.empty((src.shape[0], src.shape[1], src.shape[2]), src.dtype)
    cv.cvtColor(src, cv.COLOR_BGRA2BGR, dst)
    both = np.empty((src.shape[0], src.shape[1]*2, src.shape[2]), src.dtype)
